...
**Always check for a personal style profile first.** Before rewriting, read `~/.claude/style-profile.md` if it exists. Treat its "Voice rules (binding)" as hard constraints that override the generic defaults in PERSONALITY AND SOUL below, honor its Banned/Preferred lists, and match the register (casual vs written-work) the profile specifies. The profile is the user's own documented voice, so it wins over your guesses. If no profile exists, fall back to the sample-or-default behavior below. (Build or refine the profile with the `/style-profile` skill.)
